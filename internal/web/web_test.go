...
func TestRoutes(t *testing.T) {
	...
	t.Run("bounded fleet launch", func(t *testing.T) {
		...
		body := strings.NewReader(`{"adapter":"oci","runtime":"docker","provider":"codex","role":"implementer","repository":"frostyard/firn","source":"/repo","baseRef":"main","count":9}`)
		...
		if workers.launches[before].Adapter != worker.AdapterOCI || workers.launches[before].Runtime != worker.RuntimeDocker {
			t.Fatalf("fleet execution = %#v", workers.launches[before])
		}
	})
	...
	t.Run("launch worker", func(t *testing.T) {
		body := strings.NewReader(`{"adapter":"oci","runtime":"docker","provider":"codex","role":"implementer","repository":"frostyard/firn","source":"/repo","baseRef":"HEAD"}`)
		...
		if workers.launch.Adapter != worker.AdapterOCI || workers.launch.Runtime != worker.RuntimeDocker || workers.launch.Role != "implementer" || workers.launch.Repository != "frostyard/firn" {
